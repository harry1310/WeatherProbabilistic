"""Phase 4a per-cell — TRAINING ONLY. Fits one dbarts BART per
(station, lead) and persists per-cell state + preprocess + metadata
under data/models/precipitation/{station}/{version}_phase4a_percell/.

Mirror of train_4a.py but with the lead-pooling architectural choice
inverted: each (station, lead) gets its own BART fit on its own
~14k-row slice, with no `lead` feature column. Matches 3a's per-cell
architecture and recovers the -4.0% aggregate Brier advantage that
the lead-pooled 4a gave up (see WB bake-off 2026-05-10).

Deployment posture: per-cell ships as a CHALLENGER alongside 3a
(phases.yaml role=challenger). 3a stays Current; per-cell 4a renders
as a Models card + writes to the predictions tree but doesn't replace
the deployed champion's prediction line. Bovey-specific underperformance
is accepted at this stage (caveat documented in
WB/docs/PER_CELL_4A_PLAN.md).
...
def main() -> None:
    ...
    version = datetime.now(timezone.utc).strftime("v%Y-%m-%d_%H%M%S_phase4a_percell")
    ...
    for station_input in stations:
        ...
        # RetrainGuard against the previous per-cell 4a (or first-ever
        # if no prior). Phase tag `4a_percell` keeps this lineage
        # separate from deployed lead-pooled `4a` so the guard's
        # try_load_previous_summary returns the right baseline.
